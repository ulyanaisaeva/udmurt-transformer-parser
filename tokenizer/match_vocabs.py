...
def update_dict(vocab_pth):
    ...
    o = [output[i] for i in range(7)]
    for idx, row in enumerate(output[7:], start=7):
        s = row.split('\t')[0]
        s += "\t-" + str(idx) + '\n'
        o.append(s)
    return o
...
def match(s, dict16):
    ...
    for idx in range(len(s) - 1):
        if s[:idx] in dict16:
            ...
            if pieces:
                res.extend([s[:idx], ] + p for p in pieces)
    return res


def matcher(wiki_voc, sent_voc, out_vocab="./models/matcher/wiki_quora_match", matcher=1):
    dict16 = {}
    for line in tqdm(wiki_voc):
        tok16, id16 = [l.strip() for l in line.split('\t')]
        if tok16:
            dict16[tok16] = id16
    out = []
    ...
    with open(f'{out_vocab}_matcher_f{matcher}.tsv', 'w') as f:
        for item in out:
            f.write("%s\n" % item)
