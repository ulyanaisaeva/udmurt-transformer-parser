"""
This code originates from https://github.com/LEYADEV/Vocabulary-Transfer/blob/main/step3.py and contains minor changes.
"""
import os
import argparse
import torch
...
def get_random_embeds(config):
    random_model = BertForMaskedLM(config)
    return random_model.bert.embeddings.word_embeddings.weight
...
def main():
    ...
    model = get_pretrained_model(args.source_bert_model)
    check_dir(f'{args.save_pht}pretrain')
    model.save_pretrained(f'{args.save_pht}pretrain')

    random_embeds = get_random_embeds(conf)
    model.bert.embeddings.word_embeddings = torch.nn.Embedding.from_pretrained(random_embeds, freeze=False)
    check_dir(f'{args.save_pht}randomhead')
    model.save_pretrained(f'{args.save_pht}randomhead')

    random_model = BertForMaskedLM(conf)
    check_dir(f'{args.save_pht}random')
    random_model.save_pretrained(f'{args.save_pht}random')
