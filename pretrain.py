...
parser.add_argument('--config', type=str, default='config/pretrain_config.yaml', help='Path to config YAML file')
...
amb = test_data['validation']['labels_amb']
test_data['validation'] = test_data['validation'].remove_columns('labels_amb')
data['validation'] = test_data['validation']
...
prepared_data = data.map(lambda x: tokenize_and_align_labels(x, tokenizer, multilabel=True, last_token=False),
...
def one_hot_encode_dataset(examples, num_labels):
    def one_hot_encode_labels(example_labels, num_labels):
        zeros = np.zeros((len(example_labels), num_labels))
        for i, token_labels in enumerate(example_labels):
            if token_labels[0] == -100:
                zeros[i, :] = -100
            else:
                zeros[i, token_labels] = 1
        return zeros

    dataset_ohe = []
    for example_labels in examples["labels"]:
        sample_zeros = one_hot_encode_labels(example_labels, num_labels)
        dataset_ohe.append(sample_zeros)
    return {"labels": dataset_ohe}


data_ohe = prepared_data.map(lambda x: one_hot_encode_dataset(x, len(label_list)), batched=True, remove_columns="words")
    ...
    def torch_call(self, features):
        ...
        if padding_side == "right":
            ...
            for label in labels:
                ...
                padded = to_list(np.concatenate((np.array(label), padding)))
            ...
            for label in labels:
                padding = np.full((sequence_length - len(label), len(label[0])), self.label_pad_token_id, dtype=int)
                padded = to_list(np.concatenate((padding, np.array(label))))
                batch[label_name].append(padded)

        batch[label_name] = torch.tensor(batch[label_name], dtype=torch.int32)
        return batch
...
trainer = MultiLabelSigmoidTrainer(
    model=model,
    args=training_args,
    train_dataset=data_ohe["train"],
    eval_dataset=data_ohe["validation"],
    tokenizer=tokenizer,
